...
function SellTicket({ capturedTicket, history, ...props }) {
  ...
  function handleSubmit() {
    debugger;
    props
      .sellTicket(expeditionId, capturedTicket.seatNumber, passenger)
      .then(() => {
        history.push("/home");
      });
  }
            ...
            <tr>
              {props.tickets.map((ticket) => (
                <td
                  ...
                  onClick={(e) => onClickSelect(ticket.seatNumber)}
                >
                  {ticket.seatNumber}
                </td>
              ))}
...
const mapStateToProps = (state) => ({
  ...
  capturedExpedition: state.expeditions.capturedExpedition,
});
